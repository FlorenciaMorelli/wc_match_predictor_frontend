...
const LEX: Record<Locale, Lexicon> = {
  es: {
    ...
    lateWinner: (min, s) =>
      pick(
        [
          `Lo resolvió sobre la hora, a los ${min}'.`,
          `Se lo aseguró en el final, con el gol a los ${min}'.`,
        ],
        s
      ),
    ...
    brace: (p, s) =>
      pick([`${p} firmó un doblete.`, `${p} se anotó por partida doble.`], s),
    goalless: (s) =>
      pick(
        [
          `Mucho trámite y pocas situaciones claras.`,
          `Un duelo trabado, de pocas chances.`,
        ],
        s
      ),
    extraDetail: (pen, og) =>
    ...
    expected: (s) =>
      pick(
        [
          `En sintonía con lo que anticipaba el modelo.`,
          `Un resultado en línea con el pronóstico.`,
        ],
        s
      ),
  },
  en: {
    blowoutHeadline: (w, l, gf, ga, s) =>
      pick(
        [
          `${w} ran riot: ${gf}–${ga} against ${l}.`,
          `${w} swept past ${l} ${gf}–${ga}.`,
          `A rout for ${w}, ${gf}–${ga} over ${l}.`,
        ],
        s
      ),
    winHeadline: (w, l, gf, ga, s) =>
      pick(
        [
          `${w} took the points: ${gf}–${ga} against ${l}.`,
          `${w} beat ${l} ${gf}–${ga}.`,
          `A win for ${w}, ${gf}–${ga} over ${l}.`,
        ],
        s
      ),
    narrowHeadline: (w, l, gf, ga, s) =>
      pick(
        [
          `${w} edged it ${gf}–${ga} against ${l}.`,
          `A narrow win for ${w}, ${gf}–${ga} over ${l}.`,
          `${w} won by a single goal, ${gf}–${ga}, against ${l}.`,
        ],
        s
      ),
    drawHeadline: (a, b, g, s) =>
      pick(
        [
          `${a} and ${b} drew ${g}–${g}.`,
          `Honours even between ${a} and ${b}: ${g}–${g}.`,
        ],
        s
      ),
    goallessHeadline: (a, b, s) =>
      pick(
        [
          `${a} and ${b} played out a goalless draw.`,
          `No goals between ${a} and ${b}.`,
        ],
        s
      ),
    hatTrickHeadline: (p, w, l, gf, ga, s) =>
      pick(
        [
          `${p}'s night: ${w} routed ${l} ${gf}–${ga} on the back of a hat-trick.`,
          `${p} stole the show as ${w} beat ${l} ${gf}–${ga}.`,
        ],
        s
      ),
    comeback: (s) =>
      pick(
        [
          `A comeback after trailing at the break.`,
          `They fought back from behind to win it.`,
        ],
        s
      ),
    lateWinner: (min, s) =>
      pick(
        [
          `It was settled late, in the ${min}th minute.`,
          `The decider came near the end, on ${min}'.`,
        ],
        s
      ),
    opener: (name, min, pen, s) =>
    ...
    goalless: (s) =>
      pick(
        [
          `Plenty of midfield battle but few clear chances.`,
          `A tight, low-chance affair.`,
        ],
        s
      ),
    extraDetail: (pen, og) =>
    ...
    upset: (p, s) =>
      pick(
        [
          `A genuine upset: the model gave this outcome just ${p}.`,
          `A surprise the model rated unlikely (${p}).`,
        ],
        s
      ),
    expected: (s) =>
      pick(
        [
          `In line with what the model expected.`,
          `A result that matched the prediction.`,
        ],
        s
      ),
  },
...
export function buildMatchReport(locale: Locale, m: MatchReportInput): string {
  ...
  const winnerName =
    winnerSide === "a" ? m.teamA : winnerSide === "b" ? m.teamB : null;
  const loserName =
    winnerSide === "a" ? m.teamB : winnerSide === "b" ? m.teamA : null;
  ...
  const winnerGoals = winnerSide
    ? byMinute.filter((g) => g.team === winnerSide)
    : [];
  const lastWinnerGoal = winnerGoals.length
    ? winnerGoals[winnerGoals.length - 1]
    : null;
  ...
  const seed =
    total * 17 +
    (opener?.minute ?? 3) * 3 +
    (winnerSide === "a" ? 1 : winnerSide === "b" ? 2 : 0);

  // ── Beat 1: titular ──
  let headline: string;
  if (winnerName && loserName) {
    if (hatTrick && topScorer)
      headline = L.hatTrickHeadline(
        topScorer,
        winnerName,
        loserName,
        gf,
      ...
      headline = L.blowoutHeadline(winnerName, loserName, gf, ga, seed);
    else if (margin === 1)
      headline = L.narrowHeadline(winnerName, loserName, gf, ga, seed);
    else headline = L.winHeadline(winnerName, loserName, gf, ga, seed);
  } else {
    headline =
      total === 0
        ? L.goallessHeadline(m.teamA, m.teamB, seed)
        : L.drawHeadline(m.teamA, m.teamB, a, seed);
  }

  // ── Beat 2: cómo se dio (una historia saliente) ──
  const beats: string[] = [];
  if (comeback && winnerName) beats.push(L.comeback(seed));
  else if (lateWinner && lastWinnerGoal?.minute != null)
    beats.push(L.lateWinner(lastWinnerGoal.minute, seed));
  else if (
    !hatTrick &&
    opener &&
    opener.minute != null &&
  ...
  if (!hatTrick && brace && topScorer && beats.length === 0)
    beats.push(L.brace(topScorer, seed));
  if (margin >= 3 && (hasPenalty || hasOwnGoal))
    beats.push(L.extraDetail(hasPenalty, hasOwnGoal));

  // ── Beat 3: lectura del modelo ──
  const model = m.isUpset
    ? L.upset(pct(m.outcomeProb), seed)
    : m.outcomeProb >= 0.5
      ? L.expected(seed)
      : "";
